...
export default function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between border-b border-ink-200 bg-white/80 px-4 py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80 sm:px-6 sm:py-3.5">
      <div className="flex items-center gap-3">
        <button onClick={onMenuClick} className="rounded-lg p-1.5 text-ink-500 hover:bg-ink-100 lg:hidden dark:hover:bg-slate-800" aria-label="منو">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {user?.role === 'admin' && (
          <div className="relative hidden sm:block">
            <select
              ...
              className="appearance-none rounded-xl border border-ink-300 bg-white py-2 pl-9 pr-3.5 text-sm font-medium text-ink-700 outline-none transition focus:border-brand-400 focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
            >
              {factories.map((f) => (
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-ink-200 bg-white py-1.5 pl-2 pr-3 transition hover:bg-ink-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800"
          >
            ...
            {menuOpen && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute left-0 z-40 mt-2 w-60 overflow-hidden rounded-2xl border border-ink-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900"
                >
                  <div className="rounded-xl bg-ink-50 px-3 py-2.5 dark:bg-slate-800">
                    <div className="text-sm font-bold text-ink-800 dark:text-slate-100">
                    ...
                    {user?.factory_name && (
                      <div className="mt-1 text-[11px] text-ink-500 dark:text-slate-400">
                  ...
                  <button
                    ...
                    className="mt-1 flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium text-rose-600 transition hover:bg-rose-50 dark:hover:bg-rose-950/50"
                  >
