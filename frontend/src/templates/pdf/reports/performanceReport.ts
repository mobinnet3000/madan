...
export function buildPerformanceReport(input: PerformanceReportInput): PdfRenderOptions {
  ...
  const sectionHtml = `<div style="font-size:7px;color:#475569;line-height:1.8">
    <div style="font-weight:700;color:#0f2040;margin-bottom:4px">راهنمای جداول و بازه‌ها — عملکرد بخش تولید</div>
    <div><b>جدول ۱:</b> جزئیات هر رکورد (بازه، خط، پیمانکار و تمام خروجی‌ها).</div>
    ...
    <div><b>جدول پیمانکار:</b> تفکیک مشابه بر اساس پیمانکار.</div>
    <div style="margin-top:4px;color:#64748b">نمودارها: میله‌ای جمع/میانگین هر پارامتر به تفکیک خط، روند روزانه/هفتگی/ماهانه، توزیع سهم خطوط و پیمانکاران.</div>
  </div>`
  base.sections = [
    ...(base.sections ?? []),
...
export function buildPerformanceReportLegacy(input: { title: string; factoryName: string; dateFrom: string; dateTo: string; rows: Record<string, string | number>[]; kpis?: PdfKpiCard[]; branding?: { logoUrl?: string } }): PdfRenderOptions {
