...
export function buildProductionReport(input: ProductionReportInput): PdfRenderOptions {
  ...
  const sectionHtml = `<div style="font-size:7px;color:#475569;line-height:1.8">
    <div style="font-weight:700;color:#0f2040;margin-bottom:4px">راهنمای جداول و بازه‌ها</div>
    <div><b>جدول ۱:</b> جزئیات هر رکورد (بازه، خط، پیمانکار و تمام خروجی‌های محاسبه‌شده).</div>
    <div><b>جدول آمار کلی:</b> برای هر پارامتر تعداد، جمع کل، میانگین، کمینه و بیشینه.</div>
    <div><b>جدول تفکیک خط:</b> برای هر خط تعداد و برای هر پارامتر ستون «جمع» و «میانگین» + ردیف جمع کل.</div>
    <div><b>جداول بازه‌ای (روزانه / هفتگی / ماهانه):</b> همان تفکیک جمع/میانگین اما گروه‌بندی‌شده در هر بازهٔ تقویمی — مناسب برای مقایسهٔ روند و شناسایی دوره‌های اوج/افت.</div>
    <div><b>جدول پیمانکار:</b> مشابه تفکیک خط اما بر اساس پیمانکار.</div>
    <div style="margin-top:4px;color:#64748b">نمودارها: میله‌ای جمع و میانگین هر پارامتر به تفکیک خط، روند روزانه/هفتگی/ماهانه، توزیع سهم خطوط و پیمانکاران.</div>
  </div>`
  base.sections = [
    ...(base.sections ?? []),
...
export function buildProductionReportLegacy(input: { title: string; factoryName: string; factoryAddress?: string; dateFrom: string; dateTo: string; rows: Record<string, string | number>[]; totals?: { feed: number; product: number; tailing: number; avgEff: number | null }; branding?: { logoUrl?: string } }): PdfRenderOptions {
  const kpis = input.totals ? [
