...
export default function ActivityLog() {
  ...
  const load = () => {
    ...
    api
      .get('/accounts/activity-logs/', { params })
  ...
  return (
    <div className="animate-fade-in space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-extrabold text-ink-900">لاگ فعالیت‌ها (Audit Trail)</h1>
          <p className="text-sm text-ink-500">
            ثبت تمام عملیات کاربران برای ممیزی و پایش دسترسی‌ها
          </p>
        </div>
      ...
      <div className="card flex flex-wrap items-end gap-3 p-4">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input
            ...
            placeholder="جستجو در کاربر، مدل، شرح..."
            value={q}
        ...
        <button className="btn-ghost" onClick={() => { setQ(''); setAction('') }}>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ink-100">
                {filtered.map((l, i) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-[11px] text-ink-600">
                      {new Date(l.timestamp).toLocaleString('fa-IR')}
                    </td>
                    <td className="px-4 py-3">
                      <div className="font-medium text-ink-800">{l.username}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`badge ${ROLE_BADGE[(l.role || 'operator') as keyof typeof ROLE_BADGE] || ROLE_BADGE.operator}`}>
                        {l.role || 'اپراتور'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`badge ${ACTION_STYLE[l.action] || 'bg-ink-100 text-ink-600'}`}>
                        {ACTION_LABELS[l.action] || l.action}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-medium text-ink-700">{l.model_name}</td>
                    <td className="max-w-[280px] truncate px-4 py-3 text-ink-600" title={l.description}>
                      {l.description || l.object_repr || '—'}
                    </td>
                    <td className="px-4 py-3 text-ink-500">{l.factory_name || 'همه'}</td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
