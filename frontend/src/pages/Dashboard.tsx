  ...
  return (
    <div className="card flex items-center gap-4 p-4">
      ...
      <div className="min-w-0">
        ...
        <div className="truncate text-xl font-extrabold text-ink-800">{value}</div>
...
export default function Dashboard() {
  ...
  const [loadingChunk, setLoadingChunk] = useState(0)
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const { getLogsPage } = await import('../api/logs')
      const first = await getLogsPage({ date_from: from }, 1, 80)
      setLogs(first.results.filter((l) => lineIds.includes(l.line.id)))
      setLoadingChunk(first.results.length)
...
const stats = useMemo(() => {
    ...
    return {
      lines: selectedFactory?.lines.length ?? 0,
      devices: devices.length,
      analyzers: devices.filter((d) => d.is_analyzer).length,
      totalFeed,
      totalProduct,
      avgEff,
  ...
  const efficiencyTrend = useMemo(() => {
    ...
    logs.forEach((l) => {
      ...
      arr.push(l.efficiency)
      byDate.set(l.date, arr)
    })
    return [...byDate.entries()]
      .map(([date, vals]) => ({
        date,
        راندمان: Math.round((vals.reduce((a, b) => a + b, 0) / vals.length) * 10) / 10,
      }))
  ...
  const tonnageByLine = useMemo(() => {
    ...
    logs.forEach((l) => {
      ...
      cur.feed += l.feed_tonnage || 0
      cur.product += l.product_tonnage || 0
      cur.tailing += l.tailing_tonnage || 0
      map.set(l.line.id, cur)
    })
    return [...map.entries()].map(([id, v]) => ({
      name: selectedFactory?.lines.find((l) => l.id === id)?.name ?? '—',
      ...v,
    }))
  }, [logs, selectedFactory])

  const recentLogs = useMemo(
    () => [...logs].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 6),
    [logs],
  )

  if (fLoading) return <Loading />
  if (!selectedFactory)
    return <EmptyState title="کارخانه‌ای یافت نشد" description="ابتدا از پنل ادمین کارخانه بسازید." />

  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-extrabold text-ink-900">داشبورد کارخانه</h1>
          <p className="text-sm text-ink-500">
            نمای کلی از خطوط فرآوری، دستگاه‌ها و عملکرد {selectedFactory.name}
          </p>
        </div>
        <Link to="/logs" className="btn-outline">
          <ClipboardList className="h-4 w-4" /> مشاهده گزارش‌ها
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Kpi
          icon={<Boxes className="h-6 w-6 text-brand-600" />}
          label="خطوط فرآوری"
          value={formatNumber(stats.lines)}
          sub={`${formatNumber(stats.devices)} دستگاه`}
          accent="bg-brand-50"
        />
        <Kpi
          icon={<Gauge className="h-6 w-6 text-emerald-600" />}
          label="میانگین راندمان"
          value={formatPercent(stats.avgEff)}
          sub={`${formatNumber(stats.logCount)} گزارش ثبت‌شده`}
          accent="bg-emerald-50"
        />
        <Kpi
          icon={<TrendingUp className="h-6 w-6 text-sky-600" />}
          label="تناژ ورودی"
          value={`${formatNumber(Math.round(stats.totalFeed))} تن`}
          sub={`خروجی ${formatNumber(Math.round(stats.totalProduct))} تن`}
          accent="bg-sky-50"
        />
        <Kpi
          icon={<Cpu className="h-6 w-6 text-violet-600" />}
          label="آنالایزرها"
          value={formatNumber(stats.analyzers)}
          sub="دستگاه آنالیزور فعال"
          accent="bg-violet-50"
        />
      </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-bold text-ink-700">مدل‌سازی خطوط فرآوری</h2>
          <Link to="/lines" className="flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700">
            همه خطوط <ArrowLeft className="h-3.5 w-3.5" />
          </Link>
        </div>
        {loading ? (
          <div className="card h-40 animate-pulse" />
        ) : (
          (selectedFactory.lines ?? []).slice(0, 2).map((line) => (
            <LineFlow key={line.id} line={line} />
          ))
        )}
      </section>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="card p-4">
          <h3 className="mb-3 text-sm font-bold text-ink-700">روند راندمان در زمان</h3>
        ...
        <div className="card p-4">
          <h3 className="mb-3 text-sm font-bold text-ink-700">تناژ به تفکیک خط (مجموع)</h3>
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between border-b border-ink-100 px-4 py-3">
          <h3 className="text-sm font-bold text-ink-700">آخرین گزارش‌های عملکرد</h3>
            ...
            {recentLogs.map((l) => (
              <div key={l.id} className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-ink-700">{formatDate(l.date)}</span>
                </div>
