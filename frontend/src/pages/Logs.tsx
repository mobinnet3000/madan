...
type FormState = {
  ...
  feed_tonnage: string
  product_tonnage: string
  tailing_tonnage: string
...
function LogForm({
  ...
  editing: DeviceLog | null
}) {
  const { selectedFactory } = useFactory()

  const lineDevices = useMemo(() => {
    const line = selectedFactory?.lines.find((l) => l.id === Number(form.line))
    return line?.devices ?? []
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div>
        ...
        >
          ...
          {selectedFactory?.failure_reasons.map((f) => (
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            onChange={(e) => set('runtime_hours', e.target.value)}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) => set('downtime_hours', e.target.value)}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="label">تناژ ورودی</label>
          <input
            type="number"
            step="0.1"
            min="0"
            className="input"
            value={form.feed_tonnage}
            onChange={(e) => set('feed_tonnage', e.target.value)}
          />
        </div>
        <div>
          ...
          <input
            ...
            min="0"
            className="input"
            value={form.product_tonnage}
            onChange={(e) => set('product_tonnage', e.target.value)}
          />
        </div>
        <div>
          <label className="label">باطله</label>
          <input
            type="number"
            step="0.1"
            min="0"
            className="input"
            value={form.tailing_tonnage}
            onChange={(e) => set('tailing_tonnage', e.target.value)}
          />
        </div>
      </div>
      ...
      {editing && (
        <div className="sm:col-span-2 rounded-xl bg-ink-50 p-3 text-xs text-ink-500">
          راندمان به صورت خودکار محاسبه می‌شود: (
          {editing.line.name} · {editing.shift.name} ·{' '}
          {editing.device?.name || 'بدون دستگاه'} ·{' '}
          {editing.failure_cause?.title || 'بدون علت'})
        </div>
      )}
    </div>
  )
...
export default function Logs() {
    ...
    getLogsPage(filters, page, pageSize)
      .then((data) => {
        const filtered = lineIds.length
          ? data.results.filter((l) => lineIds.includes(l.line.id))
          : data.results
        setLogs(filtered)
  ...
  }, [selectedFactory, load])

  const openCreate = () => {
    setEditing(null)
    setForm({
      ...emptyForm,
      line: selectedFactory?.lines[0]?.id ? String(selectedFactory.lines[0].id) : '',
      shift: selectedFactory?.shifts[0]?.id ? String(selectedFactory.shifts[0].id) : '',
    })
    setModalOpen(true)
  }
  ...
  const openEdit = (log: DeviceLog) => {
    setEditing(log)
    setForm({
      line: String(log.line.id),
      shift: String(log.shift.id),
      date: log.date,
      device: log.device ? String(log.device.id) : '',
      failure_cause: log.failure_cause ? String(log.failure_cause.id) : '',
      runtime_hours: String(log.runtime_hours),
      downtime_hours: String(log.downtime_hours),
      feed_tonnage: String(log.feed_tonnage),
      product_tonnage: String(log.product_tonnage),
      tailing_tonnage: String(log.tailing_tonnage),
  ...
  const submit = async () => {
    if (!form.line || !form.shift || !form.date) {
      notify('خط، شیفت و تاریخ الزامی هستند', 'error')
      return
    }
    const payload: DeviceLogPayload = {
      ...
      runtime_hours: Number(form.runtime_hours) || 0,
      downtime_hours: Number(form.downtime_hours) || 0,
      feed_tonnage: Number(form.feed_tonnage) || 0,
      product_tonnage: Number(form.product_tonnage) || 0,
  ...
  const setFilter = (k: keyof LogFilters, v: string) => {
    setPage(1)
  ...
  return (
    <div className="animate-fade-in space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-extrabold text-ink-900">گزارش عملکرد روزانه</h1>
          <p className="text-sm text-ink-500">
            ثبت و مدیریت گزارش‌های عملکرد، توقف و تولید خطوط
          </p>
        </div>
        ...
        <EmptyState
          ...
          action={
            <button className="btn-primary mt-2" onClick={openCreate}>
              <Plus className="h-4 w-4" /> ثبت اولین گزارش
            </button>
          }
        />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-ink-100">
                {sorted.map((l) => (
                  <tr key={l.id} className="transition hover:bg-ink-50/50">
                    ...
                    <td className="px-4 py-3 text-ink-600">
                      {l.device?.name || <span className="text-ink-300">—</span>}
                      {l.failure_cause && (
                        <span className="mr-2 badge bg-rose-50 text-rose-600">
                          {l.failure_cause.title}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">{formatNumber(l.feed_tonnage)}</td>
                    <td className="px-4 py-3">{formatNumber(l.product_tonnage)}</td>
                    <td className="px-4 py-3 text-xs text-ink-500">
                      {formatNumber(l.runtime_hours)} / {formatNumber(l.downtime_hours)}
                    </td>
                    <td className="px-4 py-3">
                      <span className="font-bold text-emerald-600">
                        {formatPercent(l.efficiency)}
                      </span>
                    </td>
                    <td className="px-4 py-3">
      ...
      {!loading && totalCount > pageSize && (
        <div className="flex items-center justify-between border-t border-ink-100 px-4 py-3 text-sm">
          <span className="text-ink-500">
            نمایش {Math.min((page - 1) * pageSize + 1, totalCount)} تا {Math.min(page * pageSize, totalCount)} از {totalCount} رکورد
          </span>
          <div className="flex items-center gap-2">
            <button
              className="btn-ghost h-8 w-8 !px-0 rounded-lg disabled:opacity-40"
      ...
      <Modal
        ...
        title={editing ? 'ویرایش گزارش عملکرد' : 'ثبت گزارش عملکرد جدید'}
        ...
        footer={
          <>
            ...
            <button className="btn-primary" onClick={submit} disabled={saving}>
              {saving ? 'در حال ذخیره...' : editing ? 'ذخیره تغییرات' : 'ثبت گزارش'}
            </button>
          </>
        }
      >
      ...
      >
        <p className="text-sm text-ink-600">
          آیا از حذف این گزارش عملکرد اطمینان دارید؟ این عمل قابل بازگشت نیست.
        </p>
      </Modal>
    </div>
  )
